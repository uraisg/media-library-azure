...
const FilterOptions = () => {
  ...
  const activityFilter = ["All", "Upload", "Download"]
  const sortOption = ["dateDSC", "dateASC"]

  const [date, setDate] = useState({"StartDate": "", "EndDate": ""})

  const handleActivityFilter = (filter) => {
    filterContext.setResult([])
    const temp = { ...filterContext.active, "ActivityOption": filter }
        ...
        <FilterRight>
          ...
          >
            {activityFilter.map((item, index) => (
              <a
                key={index}
                className={filterContext.active.ActivityOption == convertActivity(item) ? "filter-option-active filterOption" : "filterOption"}
                onClick={() => handleActivityFilter(convertActivity(item))}
              >
                {item}
              </a>
            ))}
