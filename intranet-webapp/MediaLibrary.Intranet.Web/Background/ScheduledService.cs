...
namespace MediaLibrary.Intranet.Web.Background
{
    ...
    public class ScheduledService : BackgroundService
    {
        ...
        private async Task ProcessACM()
        {
            ...
            /*
             ...
             #2. With data, check all staff
	              - if staff disabled, skip
	              - if staff lastlogin date < 90, query remindersent
	  	               - if remindersent all null, 
			               - send smtp
		                   - update table (remindersent)
		              - if remindersent all full,
			               - send smtp
			               - update table (disable)
            ...
            try
            {
                Debug.WriteLine("Querying staff info");
                conn.Open();
                ...
                try
                {
                    ...
                    Debug.WriteLine("List length: " + staffList.Count);
                }
                catch (Exception ex)
                {
                    Debug.WriteLine(ex, "Got exception while retriving Staff list");
                    return;
                }
                ...
                {
                    //Get staff email
                    Debug.WriteLine(i.StaffEmail);
                    string staffEmail = i.StaffEmail;

                    //Get status
                    Debug.WriteLine(i.Status);
                    ...
                    // if staff lastlogin date< 30, query remindersent
                    var todayDate = DateTime.Now;
                    Debug.WriteLine("Compared Result: " + todayDate.Subtract(lastlogin).Days);
                    if (todayDate.Subtract(lastlogin).Days > 10) //for testing, 10 days
                    {
                        Debug.WriteLine("out!!");

                        // Only process the following actions if status is detected as Inactive
                        if (status != "Active")
                        {
                            string sql = "";

                            // Checks remindersent - if null, send smtp and update table. if all not null, update a 'disable'
                            if (firstReminderSent == "")
                            {
                                Debug.WriteLine("Will send 1st reminder");
                                //send smtp to notify
                                await SendEmail(isReminder);
                            ...
                            else //1streminder
                            {
                                ...
                                {
                                    Debug.WriteLine("Will send 2nd reminder");
                                    //send smtp to notify
                                    await SendEmail(isReminder);
                                ...
                                else //2ndreminder
                                {
                                    ...
                                    {
                                        Debug.WriteLine("Will send 3rd reminder");
                                        //send smtp to notify
                                        await SendEmail(isReminder);
                                    ...
                                    {
                                        Debug.WriteLine("Will disable acc");
                                        ...
                                        //disable user
                                        sql = ACMQueries.Queries.DisableUser;
                                        await DisableUser(sql, conn, todayDate, staffEmail);
                                    }
                                }
                            }
            ...
            /*            try
                        {
                            string sql = "select loginattempts from acmsession where userid='wongwk'";
                            int loginattempts = 0;
                            conn.Open();
                            ...
                            while (reader.Read())
                            {
                                loginattempts = reader.GetInt32(0);
                            }
                            Debug.WriteLine("Login attempts: " + loginattempts);
                        }
                        catch (Exception e)
                        {
                            Debug.WriteLine(e, "Got exception while retriving status");
                            return;
                        }*/
            ...
            /* try
             {
                 Debug.WriteLine("Querying user:");
                 conn.Open();

                 // #1: Retrieve user status
                 try
                 {
                     string sql = ACMQueries.Queries.GetStatus;
                     string status = await GetStatus(sql, conn);
                     Debug.WriteLine("Status: " + status);
                 }
                 catch (Exception e)
                 {
                     _logger.LogError(e, "Got exception while retriving status");
                     return;
                 }

                 // #2: Retrieve user last login and check validity
                 try
                 {
                     //Retrieve user last login
                     string sql = ACMQueries.Queries.GetLastLogin;
                     DateTime lastlogin = await GetLastLogin(sql, conn);
                     Debug.WriteLine("Lastlogin: " + lastlogin);

                     //Check Validity (over 90days)
                     var timeNow = System.DateTime.Now;
                     Debug.WriteLine("Days compared: " + timeNow.Subtract(lastlogin).Days);
                     if (timeNow.Subtract(lastlogin).Days > 90)
                     {
                        //if status = active, check reminders sent
        ...
        private static async Task<List<ACMCustomStaffTable>> GetStaffInfo(string sql, SqlConnection conn)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    //Adds to list
                    staffList.Add(new ACMCustomStaffTable(staffemail, status, firstremindersent, secondremindersent, thirdremindersent, default));
                }
                reader.Close();
            }
            catch (Exception ex)
        ...
        private static async Task DisableUser(string sql, SqlConnection conn, DateTime todayDate, string staffEmail)
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@todayDate", Convert.ToDateTime(todayDate.ToString(dateFormat)));
                cmd.Parameters.AddWithValue("@staffEmail", staffEmail);
                using SqlDataReader reader = cmd.ExecuteReader();
            }
            catch (Exception ex)
        ...
        private async Task SendEmail(bool isReminder)
        {
            //Setting
            var smtpClient = new SmtpClient("smtp.gmail.com") //urasmtpiz.ura.gov.sg
            {
                Port = 587, //25
                Credentials = new NetworkCredential(_appSettings.SMTPSenderEmail, _appSettings.SMTPPW),
                EnableSsl = true,
            };
            ...
            if (isReminder == true)
            {
                ...
                {
                    From = new MailAddress(_appSettings.SMTPSenderEmail),
                ...
                //Recipients
                mailMessage.To.Add(_appSettings.SMTPSenderEmail);
